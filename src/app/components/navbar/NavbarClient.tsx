  ...
  return (
    <Link
      ...
      className={`inline-flex items-center gap-2 rounded-md px-3 py-2 text-[13.5px] border transition whitespace-nowrap shrink-0
        ${active ? normalActive : normalInactive}`}
...
export default function NavbarClient({ session }: NavbarClientProps) {
  ...
  const navTheme = React.useMemo<NavTheme>(() => {
    ...
    if (isMapacheTheme) {
      return {
        ...
        signOutButton:
          "inline-flex items-center justify-center gap-2 rounded-md border border-white/20 px-3 py-2 text-[13.5px] font-medium bg-white/10 text-white hover:bg-white/20",
  ...
  const beginMapacheTransition = React.useCallback(() => {
    ...
    const targetPath = "/portal/mapache";
