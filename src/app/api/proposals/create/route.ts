...
async function getConditionsText(
  ...
  const needle = filial.trim().toUpperCase();

  for (const row of values) {
    const colA = typeof row[0] === "string" ? row[0].trim().toUpperCase() : "";
...
export async function POST(req: Request) {
  try {
    ...
    /** 2) Datos opcionales desde Sheets */
    const [conditionsText, whatsappRows] = await Promise.all([
      getConditionsText(accessToken, body.subsidiary),
      getWhatsappRows(accessToken, body.subsidiary),
    ]);
