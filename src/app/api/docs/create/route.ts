...
import { fetchWhatsappCostsRows, normalizeWhatsAppRows } from "@/lib/sheets/whatsapp";
...
async function getWhatsappRows(accessToken: string, country: string): Promise<string[][]> {
  return fetchWhatsappCostsRows(accessToken, country);
}
...
export async function POST(req: Request) {
  try {
    ...
    const [conditionsText, whatsappRows] = await Promise.all([
      ...
      getWhatsappRows(accessToken, body.country),
    ]);
