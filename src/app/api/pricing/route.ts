...
/** Normaliza (mayÃºsculas, trim, sin acentos) para comparar valores de hoja */
...
/** Convierte strings tipo "$0,095790" | "1.234,56" | "1,234.56" a nÃºmero */
function parseMoney(input: unknown): number {
  if (typeof input !== "string") return Number(input) || 0;

  // Quitar espacios, sÃ­mbolos de moneda y cualquier texto adicional (p. ej. "USD")
...
function getCachedAccessToken(userId: string): string | null {
  const entry = accessTokenCache.get(userId);
  if (!entry) return null;
  const now = Math.floor(Date.now() / 1000);
  if (entry.expiresAt - TOKEN_SAFETY_WINDOW_SECONDS <= now) {
...
function rememberAccessToken(userId: string, token: string, expiresAt?: number) {
  if (typeof expiresAt !== "number") return;
  accessTokenCache.set(userId, { token, expiresAt });
}
...
async function getAccessTokenForUser(userId: string): Promise<string> {
  ...
  if (!account) {
    throw new Error("No existe una cuenta de Google asociada al usuario.");
  }
  ...
  const hasValidStoredToken =
    ...
    account.expires_at - TOKEN_SAFETY_WINDOW_SECONDS > now;

  if (hasValidStoredToken) {
    rememberAccessToken(userId, account.access_token!, account.expires_at!);
  ...
  rememberAccessToken(userId, parsed.access_token, expiresAt);
...
async function getSheetsClientForUser(): Promise<{ sheets: sheets_v4.Sheets; accessToken: string }> {
  ...
  const accessToken = await getAccessTokenForUser(session.user.id as string);
  ...
  try {
    ...
    if (body.kind === "minutes") {
      if (isMinutesNew(body)) {
        ...
        // Intentar esquema NUEVO solo si estÃ¡n definidos (y no vacÃ­os)
        ...
        // Fallback LEGACY (pestaÃ±a "costos")
        if (variant === "out") {
