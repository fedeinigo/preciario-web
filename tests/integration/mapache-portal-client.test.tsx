...
describe("MapachePortalClient integration", () => {
  ...
  it("muestra métricas agregadas en la vista de métricas", async () => {
    const now = new Date();
    const dueSoon = new Date(now.getTime() + 3 * 24 * 60 * 60 * 1000).toISOString();
    const overdue = new Date(now.getTime() - 2 * 24 * 60 * 60 * 1000).toISOString();
    ...
    mock.method(globalThis, "fetch", async (
      ...
      if (url === "/api/mapache/insights/snapshots") {
        fetchCalls.push({ url, method });
        return new Response(JSON.stringify([]), {
          status: 200,
          headers: { "Content-Type": "application/json" },
    ...
    __setRouterState({
      pathname: "/mapache-portal/metrics",
      searchParams: new URLSearchParams(),
    });

    renderPortal(bootstrap);

    await screen.findByText("Pulso del tablero");

    const totalCardTitle = await screen.findByText("Tareas activas");
    const dueSoonCardTitle = await screen.findByText("Próximas a vencer (7 días)");
    const overdueCardTitle = await screen.findByText("Vencidas");

    const getCardValue = (titleEl: SimpleElement) => {
      const card = titleEl.parentElement;
      assert.ok(card, "card container should be present");
      const valueEl = card.querySelectorAll("div")[1] ?? null;
      assert.ok(valueEl, "card value element should be present");
      return valueEl.textContent?.replace(/\s+/g, "");
    };

    assert.equal(getCardValue(totalCardTitle), "2");
    assert.equal(getCardValue(dueSoonCardTitle), "1");
    assert.equal(getCardValue(overdueCardTitle), "1");

    await waitFor(() => {
      const snapshotCall = fetchCalls.find(
        (call) => call.url === "/api/mapache/insights/snapshots" && call.method === "GET",
      );
      assert.ok(snapshotCall, "expected insights snapshots to be requested");
    });
  });
});
